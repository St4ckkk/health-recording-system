document.addEventListener("DOMContentLoaded", () => {
    ...
    trackingForm.addEventListener("submit", (event) => {
        ...
        fetch(baseUrl + "/track-appointment", {
            ...
            .then((response) => {
                    ...
                    return response.text().then((text) => {
                        ...
                        try {
                            ...
                            if (response.status === 500) {
                                throw new Error(
                                    "Server error: The server encountered an internal error. Please check the server logs for more details.",
                                )
                ...
                if (!contentType || !contentType.includes("application/json")) {
                    ...
                    return response.text().then((text) => {
                        ...
                        throw new Error(
                            "Server returned non-JSON response: " + (text.substring(0, 100) + (text.length > 100 ? "..." : "")),
                        )
                    })
                }
            ...
            .catch((error) => {
                ...
                const errorMessage =
                    error.message || "An error occurred while tracking your appointment. Please try again later."
    ...
    function updateAppointmentDetails(appointment) {
        ...
        switch (appointment.status) {
            ...
            case "completed":
                ...
                break
            case "cancelled":
                // Show as pending confirmation unless it's confirmed cancelled
                if (appointment.status === "cancelled_confirmed") {
                    statusBadge.classList.add("status-cancelled")
                    statusBadge.innerHTML = '<i class="bx bx-x-circle mr-1"></i> Cancelled'
                } else {
                    statusBadge.classList.add("status-pending-confirmation")
                    statusBadge.innerHTML = '<i class="bx bx-time-five mr-1"></i> Cancellation Pending'
                }
                break
            ...
            case "no-show":
                ...
                break
            case "rescheduled":
                // Show as pending confirmation unless it's confirmed rescheduled
                if (appointment.status === "rescheduled_confirmed") {
                    statusBadge.classList.add("status-rescheduled")
                    statusBadge.innerHTML = '<i class="bx bx-revision mr-1"></i> Rescheduled'
                } else {
                    statusBadge.classList.add("status-pending-confirmation")
                    statusBadge.innerHTML = '<i class="bx bx-time-five mr-1"></i> Reschedule Pending'
                }
                break
    ...
    function updateTimeline(appointment) {
        ...
        completedLabel.textContent = "Appointment Completed"
        ...
        switch (appointment.status) {
            ...
            case "cancelled":
                ...
                // Show cancellation pending in the timeline
                completedLabel.textContent = "Cancellation Status"
                completedMarker.className = "timeline-marker pending-confirmation"
                completedMarker.innerHTML = '<i class="bx bx-time-five text-xs"></i>'
                completedDate.textContent = "Waiting for Confirmation"
                break

            case "cancelled_confirmed":
                // If cancelled and confirmed, show as cancelled
                if (appointment.confirmed_at) {
                    confirmedMarker.className = "timeline-marker completed"
                    confirmedMarker.innerHTML = '<i class="bx bx-check text-xs"></i>'
                    confirmedDate.textContent = appointment.confirmed_at
            ...
            case "rescheduled":
                ...
                // Show rescheduling pending in the timeline
                completedLabel.textContent = "Reschedule Status"
                completedMarker.className = "timeline-marker pending-confirmation"
                completedMarker.innerHTML = '<i class="bx bx-time-five text-xs"></i>'
                completedDate.textContent = "Waiting for Confirmation"
                break
            ...
            case "rescheduled_confirmed":
                // If rescheduled and confirmed, show as rescheduled
                if (appointment.confirmed_at) {
                    confirmedMarker.className = "timeline-marker completed"
                    confirmedMarker.innerHTML = '<i class="bx bx-check text-xs"></i>'
                    confirmedDate.textContent = appointment.confirmed_at
    ...
    function updateCancelButton(appointment) {
        ...
        if (["completed", "cancelled", "cancelled_confirmed", "no-show"].includes(appointment.status)) {
    ...
    closeRescheduleConfirmModal.addEventListener("click", () => {
        rescheduleConfirmModal.classList.remove("visible")
    })

    // Cancel reschedule confirmation
    cancelRescheduleConfirmation.addEventListener("click", () => {
        rescheduleConfirmModal.classList.remove("visible")
    })

    // Confirm reschedule confirmation (final step)
    confirmRescheduleConfirmation.addEventListener("click", () => {
