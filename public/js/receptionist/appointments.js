document.addEventListener("DOMContentLoaded", () => {
    ...
    function populateAppointmentDetails(data) {
        ...
        if (appointmentStatus) {
            // Get the status
            let status = data.appointment.status || "scheduled"
            let statusClass = ""
            ...
            appointmentStatus.textContent = displayStatus
            appointmentStatus.className = `status-badge border ${statusClass} px-1 py-1 rounded text-sm`
        }
        ...
        if (patientInsurance)
            patientInsurance.textContent = data.patient && data.patient.insurance ? data.patient.insurance : "Not provided"
    ...
    document.addEventListener("click", (event) => {
        ...
        if (
            (event.target.matches(".action-button") && event.target.textContent.includes("Reschedule")) ||
            ...
            const button = event.target.matches(".action-button") ? event.target : event.target.closest(".action-button")
        ...
        if (!button.classList.contains("disabled")) {
            button.addEventListener("click", function () {
                ...
                if (isNext) {
                    ...
                    page = Number.parseInt(activePage.textContent) + 1
                } else if (isPrev) {
                    // Get current active page and decrement
                    const activePage = document.querySelector(".pagination-btn.active")
                    page = Number.parseInt(activePage.textContent) - 1
                } else {
                    page = Number.parseInt(pageValue)
                }
                ...
                const perPage = Number.parseInt(paginationSelect.value)
    ...
    if (paginationSelect) {
        paginationSelect.addEventListener("change", function () {
            const perPage = Number.parseInt(this.value)
            console.log("Items per page changed to:", perPage)

            // Here you would typically reload the data with the new per-page setting
            // For demo purposes, we'll just update the "Showing X to Y" text
            document.querySelector(".pagination-container .text-sm.text-gray-500 span:nth-child(2)").textContent = Math.min(
                perPage,
                24,
            ) // Assuming 24 total items
        })
    }
