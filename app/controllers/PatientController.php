...
class PatientController extends Controller
{
    ...
    public function verifyCode($params)
    {
        ...
        try {
            ...
            // Debug logging
            error_log("Verification attempt - Token in params: {$token}");
            error_log("Verification attempt - Token in session: " . ($_SESSION['verify_token'] ?? 'Not set'));
            error_log("Verification attempt - Code: {$code}");
            ...
            // Use the token from URL if session token is not set or doesn't match
            // This is more resilient and handles session issues
            ...
            error_log("Verification successful - Redirecting to dashboard");
            $this->redirect('/patient/monitoring/dashboard');
        } catch (\Exception $e) {
            error_log('Code verification error: ' . $e->getMessage());
            $_SESSION['flash_message'] = 'An error occurred during verification: ' . $e->getMessage();
            $this->redirect('/patient/verify/' . $token);
        }
    }
    ...
    public function monitoringDashboard()
    {
        ...
        try {
            ...
            if (!isset($_SESSION['patient']['id']) || !isset($_SESSION['patient']['request_id'])) {
                throw new \Exception('Invalid session data');
            }

            // Get patient details first to ensure they exist
            $patientData = $this->patientModel->getPatientById($_SESSION['patient']['id']);
            if (!$patientData) {
                throw new \Exception('Patient data not found');
            }

            // Get monitoring request details
            $requestData = $this->monitoringRequestModel->getById($_SESSION['patient']['request_id']);
            if (!$requestData) {
                throw new \Exception('Monitoring request not found');
            }

            // Get vital signs and symptoms data
            $monitoringData = [
                'recent_logs' => $this->patientMonitoringLogModel->getPatientLogs(
                    $_SESSION['patient']['id'],
                    $_SESSION['patient']['request_id']
                ),
                'vital_trends' => $this->patientMonitoringLogModel->getVitalTrends(
                    $_SESSION['patient']['id'],
                    $_SESSION['patient']['request_id']
                ),
                // 'symptom_summary' => $this->patientMonitoringLogModel->getSymptomSummary(
                //     $_SESSION['patient']['id'],
                //     $_SESSION['patient']['request_id']
                // )
            ];
            ...
            $progress = [
                'start_date' => $requestData->created_at,
                'end_date' => $requestData->expires_at,
                'completion_percentage' => $this->calculateMonitoringProgress(
                    $requestData->created_at,
                    $requestData->expires_at
                )
            ];

            $doctor = $this->doctorModel->getDoctorById($requestData->doctor_id);
            ...
            $_SESSION['flash_message'] = 'Error loading monitoring dashboard: ' . $e->getMessage();
            $this->redirect('/');
        }
    }

    private function calculateMonitoringProgress($startDate, $endDate)
    {
