...
class ReceptionistController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->billingModel = new BillingRecords();
    }
        ...
        try {
            ...
            if ($success) {
                ...
                $billingData = [
                    ...
                    'service_type' => $appointment->appointment_type ?? 'checkup',
                    'description' => 'Medical ' . ucfirst($appointment->appointment_type ?? 'Consultation'),
                    'amount' => $this->billingModel->getAppointmentFee($appointment->appointment_type ?? 'checkup'),
                    'status' => 'pending',
                    ...
                    'notes' => sprintf(
                        ...
                        "Generated by: %s\n" .
                        "Payment Due: %s",
                        ucfirst($appointment->appointment_type ?? 'Medical Consultation'),
                        $appointment->appointment_type ?? 'Regular Checkup',
                        date('F j, Y', strtotime($appointment->appointment_date)),
                        date('h:i A', strtotime($appointment->appointment_time)),
                        $_SESSION['staff_name'] ?? 'Reception Staff',
                        date('F j, Y', strtotime('+7 days'))
                    )
                ];

                $billingId = $this->billingModel->createAppointmentBilling($billingData);

                if (!$billingId) {
                    error_log('Failed to create billing record for appointment ' . $appointmentId);
                }
                ...
                $this->jsonResponse([
                    'success' => true,
                    'message' => 'Appointment confirmed and billing record created successfully',
    ...
    public function appointmentDetails()
    {
        ...
        $this->view('pages/receptionist/app-details.view', [
