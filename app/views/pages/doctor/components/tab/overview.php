            ...
            <?php if (!empty($symptoms)): ?>
                <?php foreach ($symptoms as $symptom):
                    ...
                    <div class="flex items-start space-x-3 p-3 <?= $bgColor ?> rounded-lg">
                        <i class="bx <?= $isTBSymptom ? 'bx-plus-medical' : 'bx-pulse' ?> <?= $iconColor ?> text-xl mt-0.5"></i>
                        <div>
                            <h4 class="font-medium <?= $textColor ?>"><?= htmlspecialchars($symptom->name) ?>
                                <?php if ($isTBSymptom): ?>
                                    <span class="text-xs bg-amber-100 text-amber-800 px-1.5 py-0.5 rounded ml-1">TB Symptom</span>
                                <?php endif; ?>
                            </h4>
